...
//
// Known, PRE-EXISTING spec-infidelity defects (swag reflects Go structs and
// ignores custom MarshalJSON) are exercised separately, with parity checks
// against the CURRENT go-swagger client to prove they are not regressions:
//
...
import (
	...
	gen "github.com/akuity/kargo-spike"
...
//
// swagger.json models Quantity, IntOrString, and V1MicroTime structurally
// (swag reflects the Go structs and ignores their custom MarshalJSON), so the
// CURRENT (go-swagger) client cannot round-trip their real wire formats. The
// new generation pipeline blanks those three definitions to typeless schemas,
// which the generator renders as interface{} -- round-tripping anything.
// Each test proves the defect is FIXED in the new client and still present in
// the old one (i.e., strict improvement, no regression).
...
func TestSpecDefect_IntOrString(t *testing.T) {
	// Real wire format of a rollouts Metric with count: 5 -- IntOrString
	// marshals as a bare scalar, but the spec models it as an object.
...
func TestSpecDefect_Quantity(t *testing.T) {
	// Real wire format of container resources -- Quantity marshals as a
	// string, but the spec models it as an object with a Format field.
...
func TestSpecDefect_MicroTime(t *testing.T) {
	// Real wire format of an Event's eventTime -- metav1.MicroTime marshals
	// as an RFC3339 string with microseconds, but the spec models it as an
	// object with a "time.Time" property.
