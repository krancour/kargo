/*
Kargo API

REST API for Kargo

API version: v1alpha1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package kargogen

import (
	"encoding/json"
)

// checks if the CreateGenericCredentialsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CreateGenericCredentialsRequest{}

// CreateGenericCredentialsRequest struct for CreateGenericCredentialsRequest
type CreateGenericCredentialsRequest struct {
	Data *map[string]string `json:"data,omitempty"`
	Description *string `json:"description,omitempty"`
	Name *string `json:"name,omitempty"`
	Replicate *bool `json:"replicate,omitempty"`
	// Type is the Kubernetes Secret type (e.g. \"Opaque\" or \"kubernetes.io/dockerconfigjson\"). It is immutable, so it may only be set at creation time. When empty, Kubernetes defaults it to \"Opaque\".
	Type *string `json:"type,omitempty"`
}

// NewCreateGenericCredentialsRequest instantiates a new CreateGenericCredentialsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCreateGenericCredentialsRequest() *CreateGenericCredentialsRequest {
	this := CreateGenericCredentialsRequest{}
	return &this
}

// NewCreateGenericCredentialsRequestWithDefaults instantiates a new CreateGenericCredentialsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCreateGenericCredentialsRequestWithDefaults() *CreateGenericCredentialsRequest {
	this := CreateGenericCredentialsRequest{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *CreateGenericCredentialsRequest) GetData() map[string]string {
	if o == nil || IsNil(o.Data) {
		var ret map[string]string
		return ret
	}
	return *o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateGenericCredentialsRequest) GetDataOk() (*map[string]string, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *CreateGenericCredentialsRequest) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given map[string]string and assigns it to the Data field.
func (o *CreateGenericCredentialsRequest) SetData(v map[string]string) {
	o.Data = &v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *CreateGenericCredentialsRequest) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateGenericCredentialsRequest) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *CreateGenericCredentialsRequest) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *CreateGenericCredentialsRequest) SetDescription(v string) {
	o.Description = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *CreateGenericCredentialsRequest) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateGenericCredentialsRequest) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *CreateGenericCredentialsRequest) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *CreateGenericCredentialsRequest) SetName(v string) {
	o.Name = &v
}

// GetReplicate returns the Replicate field value if set, zero value otherwise.
func (o *CreateGenericCredentialsRequest) GetReplicate() bool {
	if o == nil || IsNil(o.Replicate) {
		var ret bool
		return ret
	}
	return *o.Replicate
}

// GetReplicateOk returns a tuple with the Replicate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateGenericCredentialsRequest) GetReplicateOk() (*bool, bool) {
	if o == nil || IsNil(o.Replicate) {
		return nil, false
	}
	return o.Replicate, true
}

// HasReplicate returns a boolean if a field has been set.
func (o *CreateGenericCredentialsRequest) HasReplicate() bool {
	if o != nil && !IsNil(o.Replicate) {
		return true
	}

	return false
}

// SetReplicate gets a reference to the given bool and assigns it to the Replicate field.
func (o *CreateGenericCredentialsRequest) SetReplicate(v bool) {
	o.Replicate = &v
}

// GetType returns the Type field value if set, zero value otherwise.
func (o *CreateGenericCredentialsRequest) GetType() string {
	if o == nil || IsNil(o.Type) {
		var ret string
		return ret
	}
	return *o.Type
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateGenericCredentialsRequest) GetTypeOk() (*string, bool) {
	if o == nil || IsNil(o.Type) {
		return nil, false
	}
	return o.Type, true
}

// HasType returns a boolean if a field has been set.
func (o *CreateGenericCredentialsRequest) HasType() bool {
	if o != nil && !IsNil(o.Type) {
		return true
	}

	return false
}

// SetType gets a reference to the given string and assigns it to the Type field.
func (o *CreateGenericCredentialsRequest) SetType(v string) {
	o.Type = &v
}

func (o CreateGenericCredentialsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CreateGenericCredentialsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Replicate) {
		toSerialize["replicate"] = o.Replicate
	}
	if !IsNil(o.Type) {
		toSerialize["type"] = o.Type
	}
	return toSerialize, nil
}

type NullableCreateGenericCredentialsRequest struct {
	value *CreateGenericCredentialsRequest
	isSet bool
}

func (v NullableCreateGenericCredentialsRequest) Get() *CreateGenericCredentialsRequest {
	return v.value
}

func (v *NullableCreateGenericCredentialsRequest) Set(val *CreateGenericCredentialsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableCreateGenericCredentialsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableCreateGenericCredentialsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCreateGenericCredentialsRequest(val *CreateGenericCredentialsRequest) *NullableCreateGenericCredentialsRequest {
	return &NullableCreateGenericCredentialsRequest{value: val, isSet: true}
}

func (v NullableCreateGenericCredentialsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCreateGenericCredentialsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


