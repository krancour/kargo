...
# wire). The mapping forces interface{}, which round-trips any JSON.
# (`object=any` was tested; it trips quirk 3 identically.)
rm -rf "${OUT_DIR}"
docker run --rm \
  -v "${REPO_ROOT}/pkg/x/client:/out" \
  -v "${WORK_DIR}:/work" \
  ...
  --additional-properties=packageName=generated,withGoMod=true,generateInterfaces=false,enumClassPrefix=true \
  --skip-validate-spec
...
LC_ALL=C find "${OUT_DIR}" -name 'model_*.go' \
  -exec sed -i '' 's/return any{}, false/return nil, false/g' {} +

# --- Step 4: restore go.sum ---------------------------------------------------
#
# The generator emits go.mod but not go.sum, and step 2 wiped the output dir.
(cd "${OUT_DIR}" && go mod tidy)

echo "OK: client regenerated."
