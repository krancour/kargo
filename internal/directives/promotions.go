...
type PromotionStepRunner interface {
	...
	RunPromotionStep(context.Context, *directive.PromotionStepContext) (directive.PromotionStepResult, error)
}
...
// to the environment of the PromotionStep.
func StepEnvWithOutputs(outputs directive.State) PromotionStepEnvOption {
	return func(env map[string]any) {
		env["outputs"] = outputs
	}
}
...
func StepEnvWithTaskOutputs(alias string, outputs directive.State) PromotionStepEnvOption {
	return func(env map[string]any) {
...
type PromotionResult struct {
	...
	HealthCheckSteps []directive.HealthCheckStep
...
func getTaskOutputs(alias string, state directive.State) directive.State {
	if namespace := getAliasNamespace(alias); namespace != "" {
		taskOutputs := make(directive.State)
