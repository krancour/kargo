...
// construction) and, critically, wire fidelity of the display path that
// motivated the migration -- a Stage fetched through the client and
// round-tripped into kargoapi.Stage must reproduce the server's response
// exactly, with no phantom empty objects for unset optional fields.
func TestGetClientV2(t *testing.T) {
	// A realistic Stage with MOST optional fields unset. Under the old
	// go-swagger client, this is exactly the shape that sprouted phantom
	// "promotionTemplate": {}, "health": {}, "lastPromotion": {}, etc.
	...
	// The same generated-model -> kargoapi round-trip the get commands
	...
	// What gets displayed is exactly what the server sent: nothing lost,
	// nothing invented.
