...
func (s *server) GetServiceAccount(
	...
	sa, err := s.serviceAccountsDB.Get(ctx, project, name)
	if err != nil {
		return nil, fmt.Errorf(
			"error getting Kubernetes ServiceAccount %q in project %q: %w",
			name, project, err,
		)
	}
	...
	case svcv1alpha1.RawFormat_RAW_FORMAT_JSON:
		if rawBytes, err = json.Marshal(sa); err != nil {
			return nil, fmt.Errorf("error marshaling Kargo ServiceAccount to raw JSON: %w", err)
		}
	case svcv1alpha1.RawFormat_RAW_FORMAT_YAML:
		if rawBytes, err = sigyaml.Marshal(sa); err != nil {
			return nil, fmt.Errorf("error marshaling Kargo ServiceAccount to raw YAML: %w", err)
		}
	default:
